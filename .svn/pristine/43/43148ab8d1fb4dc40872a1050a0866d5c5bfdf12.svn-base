...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		function checkForm(){
			...
			if(ch_no.value==document.getElementById("no").value){
  ...
  <body style="font-size:12px;width:100%;height:100%;overflow: auto;padding: 10px;">
			...
			<a href="#">编辑采集通道信息</a><br>			
    <form action="Gm_Channel_edit.action" method="post">
    	<table cellpadding="0" cellspacing="0" width="1020" class="senfe1">
			...
			<tr>
				<td>通道编号</td>
				<td>
					<input type="hidden" id="gm_Channel.ch_id" name="gm_Channel.ch_id" value="${gm_Channel.ch_id}"/>
					<input type="hidden" id="gm_Channel.ch_state" name="gm_Channel.ch_state" value="${gm_Channel.ch_state}"/>
					<input type="text" id="gm_Channel.ch_no" name="gm_Channel.ch_no" value="${gm_Channel.ch_no}"/>
					<input type="hidden" id="no" value="${gm_Channel.ch_no}"> 必须唯一,必填
				</td>
			</tr>
    		<tr>
				<td>通道名称</td>
				<td><input type="text" id="gm_Channel.ch_name" name="gm_Channel.ch_name" value="${gm_Channel.ch_name}"/></td>
			</tr>
			<tr>
				<td>采集设备编号</td>
				<td>
					...
					<c:forEach items="${requestScope.device_list}" var="device">						
						<c:choose>
							<c:when test="${device.dev_id == gm_Channel.dev_collectId.dev_id}">
			...
			<tr>
				<td>设备通道号</td>
				<td><input type="text" id="gm_Channel.ch_chlNo" name="gm_Channel.ch_chlNo" value="${gm_Channel.ch_chlNo}"/></td>
			</tr>
    		<tr>
				<td>传感设备编号</td>
				<td>
					...
					<c:forEach items="${requestScope.device_list}" var="device">						
						<c:choose>
							<c:when test="${device.dev_id == gm_Channel.dev_sensorId.dev_id}">
								<option value="${device.dev_id}" selected="selected">${device.dev_no}</option>
							</c:when>
							<c:otherwise>
								<option value="${device.dev_id}">${device.dev_no}</option>
			...
			<tr>
				...
				<td>
					...
					<c:forEach items="${requestScope.channeltype}" var="channeltype">						
						<c:choose>
							<c:when test="${channeltype.chtype_id == gm_Channel.chtype_id.chtype_id}">
								<option value="${channeltype.chtype_id}" selected="selected">${channeltype.chtype_displayName}(${channeltype.chtype_no})</option>
							</c:when>
							<c:otherwise>
					...
					<select id="gm_Channel.ch_procesStyle" name="gm_Channel.ch_procesStyle">
						...
						<%
							while (ch_procesStylesIterator.hasNext()) {
								...
								if(ch_procesStyle.getId().equals(String.valueOf(gm_Channel.getCh_procesStyle()))){
						%>
						<option value="<%=ch_procesStyle.getId()%>" selected="selected"><%=ch_procesStyle.getName()%></option>
						<%		}else{ %>
			...
			<tr>
				<td>存储周期</td>
				<td><input type="text" id="gm_Channel.ch_memoryPeriod" name="gm_Channel.ch_memoryPeriod" value="${gm_Channel.ch_memoryPeriod}"/></td>
			</tr>
			<tr>
				<td>小数位数</td>
				<td><input type="text" id="gm_Channel.ch_dectDig" name="gm_Channel.ch_dectDig" value="${gm_Channel.ch_dectDig}"/></td>
			</tr>
			<tr>
				<td>原数据单位</td>
				<td><input type="text" id="gm_Channel.ch_unit" name="gm_Channel.ch_unit" value="${gm_Channel.ch_unit}"/></td>
			</tr>
			<tr>
				<td>量程上限</td>
				<td><input type="text" id="ch_max" name="gm_Channel.ch_max" value="${gm_Channel.ch_max}"/></td>
			</tr>
			<tr>
				<td>量程下限</td>
				<td><input type="text" id="ch_min" name="gm_Channel.ch_min" value="${gm_Channel.ch_min}"/></td>
			</tr>
			<tr>
				<td>变化率上限</td>
				<td><input type="text" id="ch_crateMax" name="gm_Channel.ch_crateMax" value="${gm_Channel.ch_crateMax}"/></td>
			</tr>
			<tr>
				<td>校准周期</td>
				<td><input type="text" id="gm_Channel.ch_corrCyc" name="gm_Channel.ch_corrCyc" value="${gm_Channel.ch_corrCyc}"/></td>
			</tr>
			<tr>
				<td>校正公式</td>
				<td><input type="text" id="gm_Channel.ch_corrFormula" name="gm_Channel.ch_corrFormula" value="${gm_Channel.ch_corrFormula}"/></td>
			</tr>
			<tr>
				<td>校正后的单位</td>
				<td><input type="text" id="gm_Channel.ch_corrUnit" name="gm_Channel.ch_corrUnit" value="${gm_Channel.ch_corrUnit}"/></td>
			</tr>				
			<tr>
				<td>场景名称</td>
				<td>
					<input type="text" disabled="disabled" id="sceneSel" readonly="readonly" value="${gm_Channel.scene_id.scene_name}"> <a href="javascript:showMenu()">选择</a> 必填
					<input type="hidden" id="gm_Channel.scene_id.scene_id" name="gm_Channel.scene_id.scene_id" value="${gm_Channel.scene_id.scene_id}"/>
				</td>
			</tr>
			<tr>
				<td>位置编号</td>
				<td><input type="text" id="gm_Channel.ch_seat_no" name="gm_Channel.ch_seat_no" value="${gm_Channel.ch_seat_no}"/></td>
			</tr>
			<tr>
				<td>通道深度</td>
				<td><input type="text" id="gm_Channel.ch_depth" name="gm_Channel.ch_depth" value="${gm_Channel.ch_depth}"/></td>
			</tr>
			<tr>
				<td>是否对外提供服务</td>
				<td>
					<c:if test="${gm_Channel.ch_offerSer == 0}">
						<input type="radio" name="gm_Channel.ch_offerSer" value="0" checked="checked"/>否
						<input type="radio" name="gm_Channel.ch_offerSer" value="1"/>是
					</c:if>
					<c:if test="${gm_Channel.ch_offerSer == 1}">
						<input type="radio" name="gm_Channel.ch_offerSer" value="0"/>否
						<input type="radio" name="gm_Channel.ch_offerSer" value="1" checked="checked"/>是
					</c:if>					
				</td>
			</tr>
			<tr>
					...
					<select name="gm_Channel.ch_datetype" onchange="ch_datetype_onchange(this.value);">
						...
						<%
							while (ch_dateTypesIterator.hasNext()) {
								...
								if(gm_Channel.getCh_datetype()!=null && ch_dateType.getId().equals(String.valueOf(gm_Channel.getCh_datetype()))){
						%>
						<option value="<%=ch_dateType.getId()%>" selected="selected"><%=ch_dateType.getName()%></option>
						<%		}else{ %>
...
<script type="text/javascript">
ch_datetype_onchange(document.getElementById('gm_Channel.ch_datetype').value);
</script>
  </body>
</html>
