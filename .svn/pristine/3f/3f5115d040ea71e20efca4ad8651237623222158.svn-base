...
<html>
  <head>    
	...
	<script type="text/javascript">
		...
		function checkForm(){
			...
			$.getJSON("${ctx}/op_led_scene_exist.action?op_led_scene.led_id.led_id="+led_id.value+"&op_led_scene.scene_id.scene_id="+scene_id.value,{
				random:Math.random()
			},function(exist){
				if(exist==0){
		...
		//下拉树
		var zTree_save_1205;
		var scenes_save_1205 = ${scenes};
		var setting_save_1205 = {
			isSimpleData: true,
			treeNodeKey: "id",
		...
		function reloadTree() {
			hideMenu();
			zTree_save_1205 = $("#dropdownMenu").zTree(setting_save_1205, scenes_save_1205);
			zTree_save_1205.expandAll(true);			
		}
	</script>
  </head>
  ...
  <body style="font-size:12px;width:100%;height:100%;overflow: auto;padding: 10px;">
			...
			<a href="#">添加LED场景关联信息</a><br>	
		<form action="${ctx }/op_led_scene_save.action?page.pageNo=${page.pageNo}&page.pageSize=${page.pageSize}&hid_condition=${hid_condition}&hid_value=${hid_value}" method="post">       		
    		<table cellpadding="0" cellspacing="0" width="1020" class="senfe1">
				...
				<tr>
					<td>关联的场景</td>
					<td>
						<input type="text" disabled="disabled" id="sceneSel" readonly="readonly" style="width: 200px;"> <a href="javascript:showMenu()">选择</a> 必填
						<input type="hidden" id="op_led_scene.scene_id.scene_id" name="op_led_scene.scene_id.scene_id" value=""/>
					</td>
				</tr>
				<tr>
					<td>场景名称</td>
					<td>
						<input type="text" id="op_led_scene.scene_name" name="op_led_scene.scene_name" style="width: 200px;"/>
					</td>
				</tr>
				<tr>
