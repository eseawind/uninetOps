...
<html>
  <head>    
	...
	<script type="text/javascript">
		...
		function gotoPage(pageNo){
			...
			var url = "${ctx}/gm_devctrlstshistory_page.action?"+ "d="+Math.random()+"&page.pageNo=" + pageNo + "&page.pageSize=${page.pageSize}&hid_condition="+encodeURI(hid_condition)+"&hid_value="+encodeURI(hid_value);
			if(hid_beginTime!=''){
		...
		function sel_condition_onchange(){
			...
			document.getElementById("sel_value_state").style.display = 'none';
			var condition = document.getElementById("sel_condition").value;
			if(condition == 'dect_state'){
				document.getElementById("sel_value_state").style.display = 'block';
			}else{
		...
		function search(){
			...
			if(sel_condition.value=='dect_state'){
				hid_value.value = document.getElementById("sel_value_state").value;
  ...
  <body style="font-size:12px;width:100%;height:100%;overflow: auto;padding: 10px;">
  	...
  	<a href="#">控制设备状态历史管理</a> </br>
  		...
  		<tr><td style=" border: 0px;padding: 0px; font-size: 12px;">
           查询条件:<select id="sel_condition" name="sel_condition" onchange="sel_condition_onchange()">
          	...
          	<option value="dect_state">设备状态</option>
          </select>
          <input id="hid_condition" name="hid_condition" type="hidden" value="${hid_condition }">
           ...
           <input id="sel_value_no" type="text" value="${hid_value }">
           	<select id="sel_value_state" style="width:156px; display:none;">
           		<%
					List<Dect_state> dect_states = StaticDataManage.getDect_states();
					Iterator<Dect_state> dect_statesIterator = dect_states.iterator();
				%>
				<%
					while (dect_statesIterator.hasNext()) {
						Dect_state dect_state = dect_statesIterator.next();
				%>
				<option value="<%=dect_state.getId()%>"><%=dect_state.getName()%></option>
				<%
					}
				%>
           	</select>           
    ...
    <br/>
  	...
  	<table cellpadding="0" cellspacing="0" width="100%" class="senfe1" style="text-align: center;">
		<tr class="list_head">    		
    		<td>控制设备编号</td>
    		<td>状态变更时间</td>
			<td>设备状态</td>
    	...
    	<c:forEach var="gm_DevCtrlStsHistory" items="${page.result }" varStatus="n">
    		<tr>
    			<td>${gm_DevCtrlStsHistory.dect_id.dect_no }&nbsp;</td>
    			<td>${gm_DevCtrlStsHistory.decst_time }&nbsp;</td>
    			<td>${gm_DevCtrlStsHistory.dect_state_str }&nbsp;</td>
    			<td>${gm_DevCtrlStsHistory.decst_valid_str}&nbsp;</td>
    		</tr>
    	</c:forEach>
    </table>
	...
	<script type="text/javascript">
		...
		for(var i=0;i<sel_condition.options.length;i++){
			...
			if(op.value=='${hid_condition}'){
				...
				if(op.value == 'dect_state'){
					sel_condition_onchange();
					var sel_value_state = document.getElementById("sel_value_state");
					for(var j=0;j<sel_value_state.length;j++){
						var oop = sel_value_state[j];
