...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		function gotoPage(pageNo){
			...
			window.location.href = "${ctx}/tblReceive_findTblReceivePage.action?page.pageNo=" + pageNo + "&page.pageSize=10&queryValue=" + encodeURI(queryValue)+"&queryName="+queryName +"&beginTime="+beginTime+"&endTime="+endTime;
		}
	</script>
	...
	<script type="text/javascript">
		function change(value){
			var html = "";
			if(value == "flag"){
				html = "<select id=\"queryValue\"><option value=\"\">全部</option><option value=\"0\">接收的短信</option><option value=\"1\">主动控制</option><option value=\"2\">被动控制</option></select>";
			}
  ...
  <body style="font-size:12px;width:100%;height:100%;overflow: auto;padding: 10px;">
		...
		<table border="0" style="font: 12px; text-align: center;" class="senfe1" width="100%">
			...
			<c:forEach items="${page.result}" var="tblReceive">
				<tr>
					<td>${tblReceive.mobileNumber }</td>
					<td>${tblReceive.dd }</td>
					<c:choose>
						<c:when test="${tblReceive.flag == 0}">
							<td>接收的短信</td>
						</c:when>
						<c:when test="${tblReceive.flag == 1}">
							<td>主动控制</td>
						</c:when>
						<c:when test="${tblReceive.flag == 2}">
							<td>被动控制</td>
						</c:when>
						<c:otherwise>
							<td></td>
					...
					<td>
						<table class="ctl" border="0" cellSpacing="0" cellpadding="0" width="90" style="font: 12px;">
							<tr>
								<td id="tip1" style="border: 0;" title="${tblReceive.receivemsg }">${tblReceive.receivemsg }</td>
							</tr>
						</table>
					</td>
				</tr>
			...
			<tr>
				<td colspan="4">
					<table border="0" cellSpacing="0" cellpadding="0" width="100%" style="font: 12px;">
						<tr>
							<td align="left" style="border: 0;"> 
								<span style="font-size: 12px;">
									共${page.totalCount}条记录， 每页${page.pageSize}条，
									当前第${page.pageNo}页， 共${page.totalPages}页 </span>
							</td>
							<td align="right" style="font-size: 12px; border: 0;">
		...
		<script type="text/javascript">
			...
			if(queryName.value == "flag"){
				var html = "<select id=\"queryValue\"><option value=\"\">全部</option><option value=\"0\">接收的短信</option><option value=\"1\">主动控制</option><option value=\"2\">被动控制</option></select>";
				$("#values").html(html);
			}
