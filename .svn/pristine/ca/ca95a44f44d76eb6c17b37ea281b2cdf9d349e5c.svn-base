  ...
  <head>    
	...
	<script type="text/javascript">
		...
		function doSubmit(){
			...
			$.getJSON("${ctx}/op_scene_deleteChild.action?op_Scene.scene_id="+scene_id+"&scene_id_list="+scene_id_list,{
				random:Math.random()
			},function(suc){
				if(suc){
					alert("操作已成功");
  ...
  <form action="#">
		...
		<a href="#">删除场景关联</a> <br/>  
	   	<input disabled="disabled" type="hidden" id="hidden_scene_id"/><br/>
			...
			<tr>
	   			<td>场景编号：</td>
	   			<td><input disabled="disabled" type="text" id="txt_scene_no"/></td>
	   		</tr>
	   		<tr>
	   			<td>场景名称：</td>
	   			<td><input disabled="disabled" type="text" id="txt_scene_name"/></td>
	   		</tr>
	   		<tr>
	   			<td colspan="2">子场景：</td>
	   		</tr>
