...
public class Gm_DevCtrlOperateHistoryAction {
...
//不能在循环里写查询  >>>>>>>>>>>>>>>>>			
			...
			Filter filter = Filter.and(Filter.in("dect_id.dect_id", list),Filter.and(Filter.greaterOrEqual("deco_time", beginTime),Filter.lessOrEqual("deco_time", endTime)));
			if(this.hid_condition.equals("dect_no")){
				...
				filter = Filter.and(Filter.in("dect_id.dect_id", list),Filter.in("dect_id", gm_DevCtrl_nolike),Filter.and(Filter.greaterOrEqual("deco_time", beginTime),Filter.lessOrEqual("deco_time", endTime)));
			}else if(this.hid_condition.equals("deco_result")){
				filter = Filter.and(Filter.in("dect_id.dect_id", list),Filter.equal("deco_result", Integer.parseInt(this.hid_value)),Filter.and(Filter.greaterOrEqual("deco_time", beginTime),Filter.lessOrEqual("deco_time", endTime)));
			}
			search.addFilter(filter);
		}
		search.addSortAsc("dect_id");
		search.addSortDesc("deco_time");
		this.page = this.gm_DevCtrlOperateHistoryService.search(page, search);
		return "page";
	}
...
//分割线------------------------------------------------------------------------------------------
	...
	private Gm_DevCtrlOperateHistory aa = new Gm_DevCtrlOperateHistory();
	private Page<Gm_DevCtrlOperateHistory> page;
	...
	public Gm_DevCtrlOperateHistory getAa() {
		return aa;
	}
	public void setAa(Gm_DevCtrlOperateHistory aa) {
		this.aa = aa;
	}
	public Page<Gm_DevCtrlOperateHistory> getPage() {
		return page;
	}
	public void setPage(Page<Gm_DevCtrlOperateHistory> page) {
		this.page = page;
	}
