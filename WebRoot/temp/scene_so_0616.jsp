	...
	<script type="text/javascript">		
		...
		function echoSceneTree(id,name,no,rank,gtype){
			...
			if(hid_gtype.value != gtype){
				...
				if(deco_result_timer!=null){
					clearInterval(deco_result_timer);
				}
			...
			if(confirm("是否"+deco_type_cn+"增氧设备?")){			
				$.getJSON("${ctx}/gm_devctrloperate_operate.action?dect_id="+dect_id+"&deco_type="+deco_type,{
						...
						if(msm == '1') {//currentStat = 1;
							...
							deco_result_timer = window.setInterval("deco_result_fuc('"+dect_id+"')",5*1000);
						}
					}
				);
			}		
		...
		//扫描操作返回
		function deco_result_fuc(dect_id){
			$.getJSON("${ctx}/gm_devctrloperate_json_findByDect_id.action?dect_id="+dect_id,{
					random:Math.random()
				},function(deco){
					if(deco!=null){
						if(deco.deco_result != 0){							
							if(deco.deco_result==1){
								
							}else if(deco.deco_result==2){
								if(deco.deco_errorCode == 28){
									alert('控制失败，没有接电流互感器');
								}else if(deco.deco_errorCode == 21){
									alert('控制失败，电源缺项');
								}else if(deco.deco_errorCode == 19){
									alert('控制失败，电流异常');
								}else if(deco.deco_errorCode == 5){
									alert('控制失败，参数错误');
								}
							}else if(deco.deco_result==3){
								alert('超时');
							}else if(deco.deco_result==4){
								alert('控制失败，当前设备无法控制');
							}else{
								alert('控制失败');
							}
