...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <tr>
    <td valign="top">
    <form action="${ctx}/autoCtrlConfig_update.action" id="myForm">
    <input type="hidden" name="accId" value="${autoCtrlConfig.id }" />
                ...
                <td>
                    <table border="0" cellspacing="0" cellpadding="0" class="embedded_tab">
                        <tr>
                            ...
                            <td>&nbsp;${autoCtrlConfig.scene.scene_name }</td>
                            <input name="autoCtrlConfigForm.scene.scene_id" type="hidden" value="${autoCtrlConfig.scene.scene_id }"/>
                        </tr>
                    </table>
                </td>
            </tr>
                        ...
                        <tr>
                            ...
                            <td style="border-bottom:none;">
                               <select id="dectId" name="autoCtrlConfigForm.devCtrl.dect_id">
								<c:forEach items="${devCtrls }" var="devCtrl">
									<c:choose>
										<c:when test="${devCtrl.dect_id == autoCtrlConfig.devCtrl.dect_id }">
											<option selected="selected" value="${devCtrl.dect_id }">${devCtrl.dect_name}（${devCtrl.dect_no }）</option>
										</c:when>
										<c:otherwise>
                            ...
                            <td style="border-bottom:none;">
                                <select name="autoCtrlConfigForm.type">
                                    <c:if test="${autoCtrlConfig.type == 1}">
									<option selected="selected" value="1">开</option>
									<option value="2">停</option>
									<option value="3">关</option>
								</c:if>
								<c:if test="${autoCtrlConfig.type == 2}">
									<option value="1">开</option>
									<option selected="selected" value="2">停</option>
									<option value="3">关</option>
								</c:if>
								<c:if test="${autoCtrlConfig.type == 3}">
									<option value="1">开</option>
									<option value="2">停</option>
									<option selected="selected" value="3">关</option>
								</c:if>
                                </select>
                            </td>
                            ...
                            <td style="border-bottom:none;">
                                <select name="autoCtrlConfigForm.enable">
                                   <c:if test="${autoCtrlConfig.enable.title == '否'}">
									<option selected="selected" value="NOT">否</option>
                                    <option value="IS">是</option>
								</c:if>
								<c:if test="${autoCtrlConfig.enable.title == '是'}">
									<option value="NOT">否</option>
                                    <option selected="selected" value="IS">是</option>
								</c:if>
                                </select>
                            </td>
                            ...
                            <td style="border-right:none; border-bottom:none;">
                                <select name="autoCtrlConfigForm.cond">
                                    <c:if test="${autoCtrlConfig.cond.title == '并且'}">
									<option selected="selected" value="AND">并且</option>
									<option value="OR">或者</option>
								</c:if>
								<c:if test="${autoCtrlConfig.cond.title == '或者'}">
									<option value="AND">并且</option>
									<option selected="selected" value="OR">或者</option>
								</c:if>
                                </select>
                            </td>
                        </tr>
            ...
            <tr>
                <td style="border-bottom:none;">
                    <table id="condition" width="100%" border="0" cellspacing="0" cellpadding="0">
                    	<c:forEach items="${autoCtrlFactors }" var="autoCtrlFactor" varStatus="status">
                    		<tbody>
                                    ...
                                    <tr>
                                        <td style="border-bottom:none;" class="row_height">
                                            <table id="chTab" border="0" cellspacing="0" cellpadding="0" class="embedded_tab">
                                                ...
                                                <c:forEach items="${fn:split(autoCtrlFactor.channelId,',') }" var="chId">
												<tr id="chTr">
													<td>
														<select id="channel">
															<c:forEach items="${channels }" var="channel">
																<c:choose>
																	<c:when test="${channel.ch_id == chId }">
																		<option title="${channel.chtype_id.chtype_no }" selected="selected" value="${channel.ch_id }">${channel.ch_name}（${channel.ch_no }）</option>
																	</c:when>
																	<c:otherwise>
                                        ...
                                        <td style="border-bottom:none;">
                                        	<select name="autoCtrlConfigForm.handles">
                                                <c:if test="${autoCtrlFactor.handle.title == '原始值'}">
											<option selected="selected" value="ORA">原始值</option>
											<option value="AVG">平均值</option>
											<option value="MAX">最大值</option>
											<option value="MIN">最小值</option>
										</c:if>
										<c:if test="${autoCtrlFactor.handle.title == '平均值'}">
											<option value="ORA">原始值</option>
											<option selected="selected" value="AVG">平均值</option>
											<option value="MAX">最大值</option>
											<option value="MIN">最小值</option>
										</c:if>
										<c:if test="${autoCtrlFactor.handle.title == '最大值'}">
											<option value="ORA">原始值</option>
											<option value="AVG">平均值</option>
											<option selected="selected" value="MAX">最大值</option>
											<option value="MIN">最小值</option>
										</c:if>
										<c:if test="${autoCtrlFactor.handle.title == '最小值'}">
											<option value="ORA">原始值</option>
											<option value="AVG">平均值</option>
											<option value="MAX">最大值</option>
											<option selected="selected" value="MIN">最小值</option>
										</c:if>
                                        	</select>
                                        </td>
                                        <td style="border-bottom:none;">
                                        	<select name="autoCtrlConfigForm.conditions">
                                                <c:if test="${autoCtrlFactor.condition.title == '大于'}">
												<option selected="selected" value="GREATER">大于</option>
												<option value="GREATOREQUAL">大于等于</option>
												<option value="EQUAL">等于</option>
												<option value="LESS">小于</option>
												<option value="LESSOREQUAL">小于等于</option>
											</c:if>
											<c:if test="${autoCtrlFactor.condition.title == '大于等于'}">
												<option value="GREATER">大于</option>
												<option selected="selected" value="GREATOREQUAL">大于等于</option>
												<option value="EQUAL">等于</option>
												<option value="LESS">小于</option>
												<option value="LESSOREQUAL">小于等于</option>
											</c:if>
											<c:if test="${autoCtrlFactor.condition.title == '等于'}">
												<option value="GREATER">大于</option>
												<option value="GREATOREQUAL">大于等于</option>
												<option selected="selected" value="EQUAL">等于</option>
												<option value="LESS">小于</option>
												<option value="LESSOREQUAL">小于等于</option>
											</c:if>
											<c:if test="${autoCtrlFactor.condition.title == '小于'}">
												<option value="GREATER">大于</option>
												<option value="GREATOREQUAL">大于等于</option>
												<option value="EQUAL">等于</option>
												<option selected="selected" value="LESS">小于</option>
												<option value="LESSOREQUAL">小于等于</option>
											</c:if>
											<c:if test="${autoCtrlFactor.condition.title == '小于等于'}">
												<option value="GREATER">大于</option>
												<option value="GREATOREQUAL">大于等于</option>
												<option value="EQUAL">等于</option>
												<option value="LESS">小于</option>
												<option selected="selected" value="LESSOREQUAL">小于等于</option>
											</c:if>
                                            </select>
                                        </td>
                                        <td style="border-bottom:none;"><input id="trigger" name="autoCtrlConfigForm.trigger" type="text" value="${autoCtrlFactor.trigger }" /></td>
                                    </tr>				
                                </table>
                            </td>
                            <td style="border-right:none;">
