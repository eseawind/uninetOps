function moveOpt(fromObj, toObj) {// 当点击分配按钮时，获取左右的控件名
	if(jQuery("#sel_childDevNet option:selected").length > 0){
...
function del(){
	if(jQuery("#sel_childedDevNet option:selected").length > 0){
		jQuery("#sel_childedDevNet option:selected").remove();
	}else{
		alert("请选择要删除的子网络！");
	}
}
function delAllOtherOption(){
	jQuery("#sel_childedDevNet option").remove();
}
