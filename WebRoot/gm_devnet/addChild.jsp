  ...
  <head>    
	...
	<script type="text/javascript">
		...
		function echoDeviceTree(id,addr,no){		
			...
			$.getJSON("${ctx}/gm_devnet_findChildList.action?gm_DevNet.net_id="+id+"&user_id=${user.user_id}",{
				random:Math.random()
			},function(list){
				$.each(list,function(i,child){
			...
			//加载已选子网络			
			var sel_childedDevNet = document.getElementById("sel_childedDevNet");
			sel_childedDevNet.length = 0;
			$.getJSON("${ctx}/gm_devnet_findChildedList.action?gm_DevNet.net_id="+id+"&user_id=${user.user_id}",{
				random:Math.random()
			},function(list){
				$.each(list,function(i,childed){
					sel_childedDevNet.add(new Option(childed.net_no+"("+childed.net_addr+")",childed.net_id));
				})
			});
		}
		...
		function doSubmit(){
			...
			$.getJSON("${ctx}/gm_devnet_addChild.action?gm_DevNet.net_id="+net_id+"&net_id_list="+net_id_list+"&user_id=${user.user_id}",{
				random:Math.random()
			},function(suc){
				if(suc){
					alert("操作已成功");
