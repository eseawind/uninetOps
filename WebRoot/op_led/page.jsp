...
<html>
  <head>    
	...
	<script type="text/javascript">
		...
		function gotoPage(pageNo){
			...
			window.location.href = "${ctx}/op_led_page.action?"+ "d="+Math.random()+"&page.pageNo=" + pageNo + "&page.pageSize=${page.pageSize}&hid_condition=" + encodeURI(hid_condition)+"&hid_value=" + encodeURI(hid_value);
		}	
	</script>
  </head>
  ...
  <body style="font-size:12px;width:100%;height:100%;overflow: auto;padding: 10px;"> 
    	  ...
    	  <input id="hid_value" name="hid_value" type="hidden" value="${hid_value }">
    		...
    		<input type="button" class="button1" onclick="var hid_condition = document.getElementById('hid_condition').value;var hid_value = document.getElementById('hid_value').value;window.location.href = '${ctx }/op_led_toSave.action?'+'d='+Math.random()+'&page.pageNo=${page.pageNo }&page.pageSize=${page.pageSize}&hid_condition=' + encodeURI(hid_condition)+'&hid_value=' + encodeURI(hid_value);" value="添加"/>
    	<br/>
    	<table cellpadding="0" cellspacing="0" width="100%" class="senfe1">
	    	<tr class="list_head">
	    		<td style="width: 26%;">LED名称</td>
	    		<td style="width: 26%;">LED卡号</td>			
				<td style="width: 26%;">更新时间(分钟)</td>	
				<td style="width: 14%;">是否启用</td>
				<td style="width: 8%;">编辑</td>
	    	</tr>
		    <c:forEach var="op_led" items="${page.result }" varStatus="n">
	    		<tr>
	    			<td>${op_led.led_name }&nbsp;</td>
	    			<td>${op_led.led_no }&nbsp;</td>	
	    			<td>${op_led.led_interval }&nbsp;</td>
	    			<td>
						<c:choose>
							<c:when test="${op_led.led_enable==1}">
								是
							</c:when>
							<c:when test="${op_led.led_enable==0}">
								否
							</c:when>
					...
					<td>
						<a href="javascript:var hid_condition = document.getElementById('hid_condition').value;var hid_value = document.getElementById('hid_value').value;window.location.href = 'op_led_toEdit.action?op_led.led_id=${op_led.led_id }'+'&d='+Math.random()+'&page.pageNo=${page.pageNo }&page.pageSize=${page.pageSize}&hid_condition=' + encodeURI(hid_condition)+'&hid_value=' + encodeURI(hid_value);">修改</a>
					</td>
	    		</tr>
	    	</c:forEach>
        </table>
