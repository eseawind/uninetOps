...
<%@page import="org.unism.gm.domain.Gm_Device"%>
...
<html>
  <head>    
	...
	<script type="text/javascript">		
		function checkForm(){
			...
			//如果设备编号改变了，验证设备编号唯一
			if(dev_no.value!=document.getElementById("dev_no_old").value &&　dev_serial.value==document.getElementById("dev_serial_old").value){
				//验证设备编号是否唯一
			...
			//如果设备序列号改变了，验证正设备序列号唯一
			if(dev_serial.value!=document.getElementById("dev_serial_old").value && dev_no.value==document.getElementById("dev_no_old").value){
				//验证设备序列号是否唯一
		...
		var zTree_edit_0715;
		var scenes_edit_0715 = ${scenes};
		var setting_edit_0715 = {
			isSimpleData: true,
			treeNodeKey: "id",
		...
		function reloadTree() {
			hideMenu();
			zTree_edit_0715 = $("#dropdownMenu").zTree(setting_edit_0715, scenes_edit_0715);
			zTree_edit_0715.expandAll(true);			
		}
	</script>
  </head>
  ...
  <body style="font-size:12px;width:100%;height:100%;overflow: auto;padding: 10px;"> 
  	...
  	<a href="#">编辑设备信息</a> <br/> 
    <form action="${ctx }/gm_device_edit.action?page.pageNo=${page.pageNo}&page.pageSize=${page.pageSize}&scene_id=${scene_id}&hid_condition=${hid_condition}&hid_value=${hid_value}" method="post" enctype="multipart/form-data">       		
			...
			<input id="dev_no_old" type="hidden" value="${gm_Device.dev_no }">
			<input id="dev_serial_old" type="hidden" value="${gm_Device.dev_serial }">
			<table cellpadding="0" cellspacing="0" width="1020" class="senfe1">
				...
				<tr>
					...
					<td>设备序列号: </td>
					<td>
						<input id="gm_Device.dev_serial" name="gm_Device.dev_serial" value="${gm_Device.dev_serial }"/> 必须唯一,必填
					</td>
				</tr>
				<%
					Gm_Device gm_Device = (Gm_Device)request.getAttribute("gm_Device");
						...
						<select id="gm_Device.dev_btype" name="gm_Device.dev_btype" style="width:156px;">
							...
							<%
								while (dev_btypesIterator.hasNext()) {
									...
									if(dev_btype.getId().equals(String.valueOf(gm_Device.getDev_btype()))){
							%>
							<option value="<%=dev_btype.getId()%>" selected="selected"><%=dev_btype.getName()%></option>
							<%		}else{ %>
					...
					<td>
	    				...
	    				<%
							while (dev_btypesIterator.hasNext()) {
								...
								if(dev_btype.getId().equals(String.valueOf(gm_Device.getDev_btype()))){
						%>
						<input disabled="disabled" type="text" value="<%=dev_btype.getName()%>"/>
						<%		}
							}
						%> 必填
					</td>
				...
				<tr>
					...
					<td>
						<select id="gm_Device.dev_powerType" name="gm_Device.dev_powerType" style="width:156px;">
	    					<option value="-1">-- 请选择 --</option>
							<%
								String type = gm_Device.getDev_powerType();
				...
				<tr>
					...
					<td>所属场景: </td>	
					<td>
						<input type="text" disabled="disabled" id="sceneSel" value="${gm_Device.scene_id.scene_name}"> <a href="#" onclick="showMenu()">选择</a> 必填
						<input type="hidden" id="gm_Device.scene_id.scene_id" name="gm_Device.scene_id.scene_id" value="${gm_Device.scene_id.scene_id }"/>
					</td>
				</tr>
				<tr>
					<td style="width:12px; border-right: 0px;">&nbsp;</td>
				...
				<tr>
					...
					<td>购买时间: </td>
					<td>
						<input name="gm_Device.dev_buyDate" onfocus="setday(this,this)" readonly="readonly" value="<fmt:formatDate value='${gm_Device.dev_buyDate }' pattern='yyyy-MM-dd'/>"/>
					</td>
				</tr>
				<tr>
					<td style="width:12px; border-right: 0px;">&nbsp;</td>
					<td>服务到期时间: </td>
					<td>
						<input name="gm_Device.dev_overDate" onfocus="setday(this,this)" readonly="readonly" value="<fmt:formatDate value='${gm_Device.dev_overDate }' pattern='yyyy-MM-dd'/>"/>
					</td>
				</tr>
				<tr>
					<td style="width:12px; border-right: 0px;">&nbsp;</td>
					<td>生效时间: </td>
					<td>
						<input name="gm_Device.dev_effectTime" onfocus="setday(this,this)" readonly="readonly" value="<fmt:formatDate value='${gm_Device.dev_effectTime }' pattern='yyyy-MM-dd'/>"/>
					</td>
				</tr>
				<tr>
					<td style="width:12px; border-right: 0px;">&nbsp;</td>
